...
subtitle_group_key() {
  ...
  lang="und"
  local last_lc="${last,,}"
  if [[ "$last_lc" =~ ^[a-z]{2,3}$ ]]; then
    # Check if this is a subtitle qualifier (HI/SDH/CC) rather than a language
    case "$last_lc" in
      hi|sdh|cc)
        # Look one segment back for the real language
        local before_last="${no_forced%.*}"
        local prev="${before_last##*.}"
        local prev_lc="${prev,,}"
        if [[ "$prev_lc" =~ ^[a-z]{2,3}$ ]]; then
          # Found a language before the qualifier
          lang="$(normalize_lang_code "$prev_lc")"
        else
          # No language before it — treat as language code (e.g., hi=Hindi)
          lang="$(normalize_lang_code "$last_lc")"
        fi
        ;;
      *)
        lang="$(normalize_lang_code "$last_lc")"
