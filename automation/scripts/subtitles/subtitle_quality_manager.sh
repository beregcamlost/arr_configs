...
nuclear_redownload() {
  ...
  if is_tv_path "$mkv_file"; then
    ...
    arr_id="$(sqlite3 "$BAZARR_DB" "PRAGMA busy_timeout=5000; SELECT sonarrEpisodeId FROM table_episodes WHERE path GLOB '*$(sql_escape "$(basename "${mkv_file%.*}")")*' LIMIT 1;" </dev/null 2>/dev/null)" || arr_id=""
    ...
    arr_id="$(sqlite3 "$BAZARR_DB" "PRAGMA busy_timeout=5000; SELECT radarrId FROM table_movies WHERE path GLOB '*$(sql_escape "$(basename "${mkv_file%.*}")")*' LIMIT 1;" </dev/null 2>/dev/null)" || arr_id=""
