...
STATE_DB="${STATE_DB:-${PIPELINE_DB:-$STATE_DIR/recovery_state.db}}"
...
ARR_RETRY_COOLDOWN_SEC="${ARR_RETRY_COOLDOWN_SEC:-1800}"          # 30min
...
if [[ -z "$(sqlite3 "$STATE_DB" "SELECT 1 FROM pragma_table_info('recovery_state') WHERE name='arr_attempts' LIMIT 1;")" ]]; then
  sqlite3 "$STATE_DB" "ALTER TABLE recovery_state ADD COLUMN arr_attempts INTEGER NOT NULL DEFAULT 0;"
fi
if [[ -z "$(sqlite3 "$STATE_DB" "SELECT 1 FROM pragma_table_info('recovery_state') WHERE name='regrab_attempts' LIMIT 1;")" ]]; then
  sqlite3 "$STATE_DB" "ALTER TABLE recovery_state ADD COLUMN regrab_attempts INTEGER NOT NULL DEFAULT 0;"
fi
if [[ -z "$(sqlite3 "$STATE_DB" "SELECT 1 FROM pragma_table_info('recovery_state') WHERE name='last_regrab_ts' LIMIT 1;")" ]]; then
  sqlite3 "$STATE_DB" "ALTER TABLE recovery_state ADD COLUMN last_regrab_ts INTEGER NOT NULL DEFAULT 0;"
fi
...
if [[ "$REPORT_MODE" -eq 1 ]]; then
  ...
  [[ -n "$last_regrab" ]] || last_regrab="never"
...
# Deduplicate arr search / regrab per media file (not per language)
declare -A arr_searched_this_run
declare -A regrabbed_this_run
...
process_item() {
  ...
  for item in "${miss[@]}"; do
    ...
    # Skip items that have exhausted all recovery stages
    ...
    if should_run_with_cooldown "$last_arr" "$ARR_RETRY_COOLDOWN_SEC" "$now_ts"; then
      ...
      local arr_dedup_key="${media_type}_${media_id}"
      if [[ -z "${arr_searched_this_run[$arr_dedup_key]+x}" ]]; then
        if [[ "$media_type" == "episode" ]]; then
          arr_http="$(trigger_arr_search_episode "$media_id")"
        else
          arr_http="$(trigger_arr_search_movie "$media_id")"
        fi
        arr_searched_this_run["$arr_dedup_key"]=1
    ...
    if [[ "$regrab_att" -ge "$MAX_REGRABS" && "$MAX_REGRABS" -gt 0 && "$arr_att" -ge "$MAX_ARR_ATTEMPTS" ]]; then
      if [[ "$last_result_val" != "exhausted_all_stages" ]]; then
        ...
        add_action "EXHAUSTED $media_type $media_id lang=$token regrabs=$regrab_att"
      fi
    fi
  done
}
...
# ---------------------------------------------------------------------------
if [[ $((bazarr_attempts + translations + arr_triggers + regrab_triggers)) -gt 0 ]]; then
